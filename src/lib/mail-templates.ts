...
export function buildLeadRequestPublishedMail(input: {
  name?: string;
  ...
  const intro = greeting(input.name);
  const subject = "Potwierdzenie publikacji zapytania ofertowego";
  ...
  return {
    ...
    text: renderTextLayout({
      intro,
      sections: [
        "Twoje zapytanie ofertowe zostało opublikowane.",
        `Typ: ${input.leadTypeLabel}\nOpis: ${input.descriptionPreview}`,
        ...(input.boardUrl ? [`Zapytania ofertowe: ${input.boardUrl}`] : []),
      ],
    }),
    html: renderHtmlLayout({
      preheader: "Twoje zapytanie ofertowe zostało opublikowane.",
      title: subject,
      intro,
      contentHtml:
        htmlParagraph("Twoje zapytanie ofertowe zostało opublikowane.") +
        htmlInfoPanel(
          `${htmlLabeledValue("Typ", input.leadTypeLabel)}<br/>${htmlLabeledValue("Opis", input.descriptionPreview)}`,
        ) +
        (input.boardUrl ? htmlButton(input.boardUrl, "Przeglądaj zapytania") : ""),
    }),
  };
}
