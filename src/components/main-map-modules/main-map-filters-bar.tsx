...
function SearchIcon() {
  return (
    <svg viewBox="0 0 20 20" fill="none" className="h-4 w-4 text-neutral-500" aria-hidden>
...
export function MainMapFiltersBar({
  ...
  return (
    <div className="mx-auto mt-4 w-full max-w-[1080px] px-6">
      <div className="flex flex-wrap items-center gap-1 rounded-xl border border-neutral-700 bg-neutral-900 px-1 py-1 shadow-[0_4px_18px_rgba(2,6,23,0.4)]">
        <label className="flex flex-1 items-center gap-2 rounded-lg px-3 md:min-w-[220px]">
          ...
          <input
            className="h-10 w-full bg-transparent text-sm text-neutral-100 placeholder:text-neutral-500 focus:outline-none"
        ...
        {showLocationFilter ? (
          <>
            <span className="hidden h-7 w-px bg-neutral-700 lg:block" />
            ...
            <label className="hidden items-center gap-2 rounded-lg px-3 lg:flex">
              ...
              <select
                className="h-10 rounded-lg border-0 bg-transparent px-2 pr-7 text-sm text-neutral-100 focus:outline-none [color-scheme:dark]"
        ...
        {showSort ? (
          <>
            <span className="hidden h-7 w-px bg-neutral-700 lg:block" />
            <label className="hidden items-center gap-2 rounded-lg px-3 lg:flex">
              <span className="text-sm text-neutral-300">{sortLabel}</span>
              <select
                className="h-10 rounded-lg border-0 bg-transparent px-2 pr-7 text-sm text-neutral-100 focus:outline-none [color-scheme:dark]"
                value={sortValue}
                onChange={(event) => {
              ...
              >
                <option value="newest" className="bg-neutral-950 text-neutral-100">{sortNewestLabel}</option>
                <option value="oldest" className="bg-neutral-950 text-neutral-100">{sortOldestLabel}</option>
              </select>
            </label>
          </>
        ) : null}
        <button
          type="button"
          className="hidden h-10 w-10 items-center justify-center rounded-lg border border-neutral-700 bg-neutral-950 text-neutral-300 hover:border-neutral-500 hover:text-neutral-100 disabled:cursor-not-allowed disabled:opacity-40 lg:inline-flex"
        ...
        >
          ...
          {activeAdditionalFiltersCount > 0 ? (
            <span className="absolute -top-1 -right-1 inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-sky-500 px-1 text-[10px] font-semibold text-neutral-950 lg:hidden">
              {activeAdditionalFiltersCount}
            </span>
          ) : null}
        </button>
