...
type ContainerRentSeoHubPageProps = {
  searchParams: Promise<Record<string, string | string[] | undefined>>;
};

export async function generateMetadata({
  searchParams,
}: ContainerRentSeoHubPageProps): Promise<Metadata> {
  ...
  const total = await getSeoContainerKindTotalCount("rent");
  return getContainerSeoHubMetadata(locale, "rent", total >= 3);
}

export default async function ContainerRentSeoHubPage({
  searchParams,
}: ContainerRentSeoHubPageProps) {
  const params = await searchParams;
  const cookieStore = await cookies();
  const locale = getLocaleFromRequest({
    params,
      ...
      <section className="grid gap-4 rounded-md border border-neutral-300 bg-white p-6 shadow-sm">
        ...
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {CONTAINER_SEO_CITIES.map((city) => (
            <Link
              key={city.slug}
              href={getContainerSeoCityPath(city.slug, "rent")}
      ...
      <section className="grid gap-4 rounded-md border border-neutral-300 bg-white p-6 shadow-sm">
        ...
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {CONTAINER_SEO_COUNTRIES.map((country) => (
            <Link
              key={country.slug}
              href={getContainerSeoCountryPath(country.slug, "rent")}
              className="rounded-md border border-neutral-200 bg-neutral-50 px-4 py-3 text-sm font-medium text-neutral-800 transition hover:border-neutral-300 hover:bg-neutral-100"
            >
              {country.name}
            </Link>
