  ...
  return (
    <main className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6 sm:px-6">
      <header className="rounded-xl border border-slate-800 bg-slate-900/60 p-5">
        <h1 className="text-2xl font-semibold sm:text-3xl">Podgląd template maili</h1>
        <p className="mt-2 text-sm text-slate-300">
          Wizualny podgląd HTML i wersji tekstowej na zamockowanych danych.
        </p>
        <div className="mt-4">
          <Link
      ...
      <section className="grid gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
        <aside className="rounded-xl border border-slate-800 bg-slate-900/60 p-3">
          <p className="px-2 pb-2 text-xs uppercase tracking-[0.12em] text-slate-400">Szablony</p>
            ...
            {cases.map((mailCase) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="mt-0.5 block text-xs text-slate-400">{mailCase.description}</span>
                </Link>
              );
            })}
          </nav>
        ...
        <div className="flex min-w-0 flex-col gap-4">
          <article className="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
            <p className="text-xs uppercase tracking-[0.12em] text-slate-400">Temat</p>
            <p className="mt-1 text-lg font-semibold text-slate-100">{selectedCase.content.subject}</p>
            <p className="mt-3 text-xs uppercase tracking-[0.12em] text-slate-400">Mockowany odbiorca</p>
            <p className="mt-1 text-sm text-slate-300">{selectedCase.mockedRecipient}</p>
          </article>

          <article className="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
            <p className="mb-3 text-xs uppercase tracking-[0.12em] text-slate-400">HTML</p>
          ...
          <article className="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
            <p className="mb-3 text-xs uppercase tracking-[0.12em] text-slate-400">Wersja tekstowa</p>
            <pre className="overflow-x-auto whitespace-pre-wrap rounded-lg border border-slate-700 bg-slate-950 p-3 text-xs leading-6 text-slate-200">
